...
# Shift u1 so the minimum is 0
    u1_shifted = u1_tri - np.min(u1_tri)

# Plot using shifted values
    c1 = ax.tripcolor(
        x, y, tri_plot,
        facecolors=u1_shifted,
        shading='flat',
        cmap='viridis_r',
        edgecolor='black',
        linewidth=0.5,
    ...
    plt.close()
     ...
     # Plot 5: Vector field (quiver plot) with cylinder outlined
    fig, ax = plt.subplots(figsize=(8, 8))
    step = 3
    xx = x.reshape(-1)[::step]
    yy = y.reshape(-1)[::step]
    uu = u1.reshape(-1)[::step]
    vv = u2.reshape(-1)[::step]

    r = np.sqrt(x**2 + y**2)
    cyl_r = np.min(r[r > 0])   
    theta = np.linspace(0, 2*np.pi, 200)
    cx = cyl_r * np.cos(theta)
    cy = cyl_r * np.sin(theta)

    # Plot cylinder
    ax.plot(cx, cy, 'k', linewidth=2)

    ax.quiver(xx, yy, uu, vv, angles='xy', scale_units='xy', scale=0.15, width=0.003)

    ax.set_aspect('equal')
    ax.set_title('Velocity Vector Field (Flow Arrows)', fontsize=14, fontweight='bold')
    ax.set_xlabel('x', fontsize=12)
    ax.set_ylabel('y', fontsize=12)
    ax.set_xlim(-1, 1)
    ax.set_ylim(-1, 1)
    ...
    plt.tight_layout()
    plt.savefig('velocity_quiver.png', dpi=300, bbox_inches='tight')
    print("Saved: velocity_quiver.png")
    plt.close()
